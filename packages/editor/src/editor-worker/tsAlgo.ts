...
export const sortNodesTs = ({
  ...
  let modifiedNodes = 0;
  const numberNodes = nodes.map((n: any) => {
    const existingNode = existingNumberNodes?.find(
      (enn: any) => enn.id === n.id
    );
    if (existingNode) {
      exisitingNodes += 1;
      ...
      Math.max(
        ...n.args.map((a: any) => {
          ...
          const fieldArgumentsLengths = fieldArguments.map(
            (fa: any) => fa.name.length + compileType(fa.type.fieldType).length
          );
          const fieldArgumentCumulated = fieldArguments.length
            ? fieldArgumentsLengths.reduce((a: any, b: any) => a + b, 2)
            : 0;
          return (
