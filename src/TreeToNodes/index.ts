...
export class TreeToNodes {
   ...
   *
   * @static
   * @param {(GraphQLNodeParams | undefined)} d1 Params of node #1
   * @param {(GraphQLNodeParams | undefined)} d2 Params of node #2
   ...
   *
   * @param {ParserField} n ParserField
   * @param {Node} rootNode node to be connected with
   * @param {Link[]} links already existing links
   * @param {EditorNodeDefinition[]} nodeDefinitions already existing node definitions
   * @param {Array<Node<GraphQLNodeParams>>} nodes already existing nodes
   * @returns {Node<GraphQLNodeParams>}
   * @memberof TreeToNodes
   */
   ...
   *
   * @param {string[]} interfaces interfaces names to be implemented and connected with node
   * @param {Node} rootNode node to be connected with
   * @param {EditorNodeDefinition[]} nodeDefinitions already existing nodeDefinitions
   * @param {Link[]} links already existing links
   * @param {Array<Node<GraphQLNodeParams>>} nodes already existing nodes
   * @memberof TreeToNodes
   */
   ...
   *
   * @static
   * @param {ParserField} root ParserField
   * @param {EditorNodeDefinition[]} nodeDefinitions already existing nodeDefinitions
   * @param {Array<Node<GraphQLNodeParams>>} nodes already existing nodes
   * @param {Link[]} links already existing links
   * @param {Node} [rootNode] node to be connected with
   ...
   *
   * @static
   * @param {ParserTree} tree tree generated by Parser from 'graphql-zeus'
   * @param {EditorNodeDefinition[]} nodeDefinitions predefined graphql definitions
   * @returns {{
   *     nodes: Array<Node<GraphQLNodeParams>>;
   *     links: Link[];
